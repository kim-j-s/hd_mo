...
(function (global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? (module.exports = factory()) : typeof define === "function" && define.amd ? define(factory) : ((global = typeof globalThis !== "undefined" ? globalThis : global || self), (global.CupertinoPane = factory()));
})(this, function () {
	"use strict";
    ...
    ***************************************************************************** */
	...
	class Device {
		constructor() {
			...
			// Webview
			this.webView = !!((iphone || ipad || ipod) && (ua.match(/.*AppleWebKit(?!.*Safari)/i) || window.navigator["standalone"])) || (window.matchMedia && window.matchMedia("(display-mode: standalone)").matches);
	...
	class Events {
		...
		touchStart(t) {
			...
			if (this.contentScrollTop && this.willScrolled() && !this.isDraggableElement(t)) {
		...
		touchMove(t) {
			...
			if (this.steps.length > 2) {
				if (this.isFormElement(document.activeElement) && !this.isFormElement(t.target)) {
			...
			if (this.settings.touchAngle && !this.isScrolling) {
				...
				if (diffX * diffX + diffY * diffY >= 25 && 90 - touchAngle > this.settings.touchAngle && this.steps.length === 1) {
			...
			// Scroll handler
			if (this.instance.overflowEl.style.overflowY === "auto" && this.scrollPreventDrag(t) && !this.isDraggableElement(t)) {
				return;
			}
			...
			// No changes Y/X
			if (this.instance.getPanelTransformY() === newVal && this.instance.getPanelTransformX() === newValX) {
				return;
			}
		...
		touchEnd(t) {
			...
			if (this.fastSwipeNext("Y")) {
				closest = this.instance.swipeNextPoint(
					((_a = this.steps[this.steps.length - 1]) === null || _a === void 0 ? void 0 : _a.posY) - ((_b = this.steps[this.steps.length - 2]) === null || _b === void 0 ? void 0 : _b.posY), //diff
					this.swipeNextSensivity,
					closest,
				);
				fastSwipeClose = this.settings.fastSwipeClose && this.breakpoints.currentBreakpoint < closest;
			}
			...
			if (this.isFormElement(document.activeElement) && !this.isFormElement(t.target) && this.steps.length === 2) {
				blurTapEvent = true;
			}
			// Event emitter
		...
		handleSuperposition(coords) {
			...
			if (!this.settings.upperThanTop && (coords.newVal <= this.breakpoints.topper || (coords.clientY <= this.breakpoints.topper && !this.settings.zStack))) {
				this.steps = [];
			 ...
			 */
			if (this.settings.upperThanTop && (coords.newVal <= this.breakpoints.topper || this.startPointOverTop)) {
			...
			if (!this.settings.lowerThanBottom && coords.newVal >= this.breakpoints.bottomer) {
		...
		willScrolled() {
			if (!(this.isElementScrollable(this.instance.overflowEl) && this.instance.overflowEl.style.overflow !== "hidden")) {
				return false;
			}
			return true;
		}
		isDraggableElement(t) {
			return t.target === this.instance.draggableEl || t.target === this.instance.moveEl;
		}
		isFormElement(el) {
			const formElements = ["input", "select", "option", "textarea", "button", "label"];
	...
	class KeyboardEvents {
		...
		isOnViewport() {
			if (this.instance.paneEl && this.instance.paneEl.offsetWidth === 0 && this.instance.paneEl.offsetHeight === 0) {
				return false;
			}
			return true;
		}
		...
		getActiveInputClientBottomRect() {
			var _a, _b;
			if (document.activeElement.classList.contains("native-textarea") || document.activeElement.classList.contains("native-input")) {
			...
			return new Promise(resolve =>
				__awaiter(this, void 0, void 0, function* () {
					...
					const transitionEnd = () => {
						...
						if (params.type === CupertinoTransition.Breakpoint || params.type === CupertinoTransition.Present || params.type === CupertinoTransition.TouchEnd) {
							this.isPaneHidden = false;
						}
						// toggle ion-content scroll-y
						if ((params.type === CupertinoTransition.Hide || params.type === CupertinoTransition.Destroy) && this.instance.ionContent && !this.settings.ionContentScroll && !this.doesPanesExists()) {
					...
					if (params.type === CupertinoTransition.Breakpoint || params.type === CupertinoTransition.TouchEnd || params.type === CupertinoTransition.Present || params.type === CupertinoTransition.Hide || params.type === CupertinoTransition.Destroy) {
						// Allow custom transitions for present/destroy
						// + Fix arguments mutations from re-call present
						let subTransition = ((_a = params.conf) === null || _a === void 0 ? void 0 : _a.transition) ? JSON.parse(JSON.stringify(params.conf.transition)) : {};
						// freemode
	...
	class ZStackModule {
		constructor(instance) {
			...
			this.instance["setZstackConfig"] = zStack =>
				__awaiter(this, void 0, void 0, function* () {
					return this.setZstackConfig(zStack);
				});
		...
		pushTransition(pushElement, newPaneY, transition) {
			...
			const topHeight = this.settings.zStack.minPushHeight ? this.settings.zStack.minPushHeight : this.instance.screenHeightOffset - this.breakpoints.bottomer;
	...
	class BackdropModule {
      ...
      `);
			...
			this.instance.on("onTransitionStart", ev => {
				...
				if (this.instance.isHidden() || ev.type === CupertinoTransition.Hide || ev.type === CupertinoTransition.Destroy || ev.type === CupertinoTransition.Present) {
					...
					if (ev.type !== CupertinoTransition.Hide && ev.type !== CupertinoTransition.Destroy) {
						this.backdropEl.style.display = "block";
						setTimeout(() => {
			...
			this.instance.on("onTransitionEnd", ev => {
				...
				if (ev.type === CupertinoTransition.Destroy || ev.type === CupertinoTransition.Hide) {
		...
		isBackdropPresented() {
			return document.querySelector(`.cupertino-pane-wrapper .backdrop`) ? true : false;
		}
		touchMoveBackdrop(t) {
			if (this.settings.touchMoveStopPropagation) {
				t.stopPropagation();
	...
	class FitHeightModule {
		constructor(instance) {
			...
			// TODO: change binding strategy according to TypeScript
			// E.G. Using public module methods from modules
			this.instance["calcFitHeight"] = animated =>
				__awaiter(this, void 0, void 0, function* () {
					return this.calcFitHeight(animated);
				});
		...
		setOverflowHeight(offset = 0) {
			if (this.paneElHeight > this.instance.screen_height) {
				...
				this.instance.overflowEl.style.height = `${this.instance.getPaneHeight() - this.settings.topperOverflowOffset - this.instance.overflowEl.offsetTop - offset}px`;
			}
		}
		getPaneFitHeight() {
			return __awaiter(this, void 0, void 0, function* () {
	...
	class InverseModule {
      ...
      `);
		...
		handleSuperposition(coords) {
			// Inverse gestures
			// Allow drag topper than top point
			if (this.settings.upperThanTop && (coords.newVal >= this.breakpoints.topper || this.events.startPointOverTop)) {
				// check that finger reach same position before enable normal swipe mode
			...
			if (!this.settings.upperThanTop && coords.newVal >= this.breakpoints.topper) {
				return { y: this.breakpoints.topper };
			}
		}
		scrollPreventDrag(t) {
	...
	class HorizontalModule {
		...
		static CollectSettings(settings) {
			return settings.horizontal ? Object.assign(Object.assign({}, settings), HorizontalModule.forceSettings) : settings;
		}
		constructor(instance) {
			this.instance = instance;
			this.settings = this.instance.settings;
			...
			this.instance.on("onTransitionEnd", ev => {
				if ((ev.type === "breakpoint" || ev.type === "present") && !this.instance.getPanelTransformX()) {
		...
		setPaneElTransform(params) {
			...
			if (params.type === "end") {
				...
				if (this.fastSwipeNext) {
					if (this.currentBreakpoint === "left" && this.instance.getPanelTransformX() > this.horizontalBreaks[0]) {
						closest = this.horizontalBreaks[1];
					}
					if (this.currentBreakpoint === "right" && this.instance.getPanelTransformX() < this.horizontalBreaks[1]) {
	...
	class ModalModule {
		// Force to use settings by module
		static CollectSettings(settings) {
			return settings.modal ? Object.assign(Object.assign({}, settings), ModalModule.ForceSettings) : settings;
		}
		constructor(instance) {
			this.instance = instance;
			this.modalDefaults = {
			...
			// set defaults
			this.settings.modal = typeof this.settings.modal === "object" ? Object.assign(Object.assign({}, this.modalDefaults), this.settings.modal) : this.modalDefaults;
      ...
      `);
		...
		handleSuperposition(coords) {
			...
			if (this.settings.modal["dismissOnIntense"] && (differY > intenseKoeff || differX > intenseKoeff - 10)) {
	...
	class CupertinoPane {
		constructor(selector, conf = {}) {
			...
			if (this.settings.parentElement) {
				parentElement = this.settings.parentElement instanceof HTMLElement ? this.settings.parentElement : document.querySelector(this.settings.parentElement);
			}
    ...
    `;
		...
		present(conf = { animate: false }) {
			...
			return __awaiter(this, void 0, void 0, function* () {
				 ...
				 */
				let customTransitionFrom = ((_a = conf === null || conf === void 0 ? void 0 : conf.transition) === null || _a === void 0 ? void 0 : _a.from) ? JSON.parse(JSON.stringify(conf.transition.from)) : null;
		...
		setOverflowHeight(offset = 0) {
			...
			this.overflowEl.style.height = `${this.getPaneHeight() - this.settings.topperOverflowOffset - this.overflowEl.offsetTop - offset}px`;
		}
